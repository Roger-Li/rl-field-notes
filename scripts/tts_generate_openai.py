#!/usr/bin/env python3
"""Generate an MP3 narration from a plain-text transcript using OpenAI gpt-4o-mini-tts.

Same CLI interface as tts_generate.py / tts_generate_api.py:
  --input, --output, --lang, --voice
"""
...
DEFAULT_VOICE = "nova"  # Natural female English voice
...
def generate(input_path: str, output_path: str, lang: str, voice: str) -> None:
    ...
    if not api_key:
        print("OPENAI_API_KEY not set. Cannot generate.", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"Generating via OpenAI {MODEL} ({len(text)} chars, voice={voice})")
    ...
    tmp = Path(tempfile.mktemp(suffix=".mp3", prefix="tts_openai_"))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate TTS audio via OpenAI gpt-4o-mini-tts")
