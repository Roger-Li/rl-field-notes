...
Use the `/new-article` slash command to scaffold all boilerplate files automatically. It creates route wrappers, content component stubs, transcript stubs, registers the entry in `lib/content.ts`, and should also update the curated About timeline entry for the release.
